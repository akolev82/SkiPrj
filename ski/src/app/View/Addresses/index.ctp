<div class="addresses index">
	<h2><?php echo __('Addresses'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('AddressID'); ?></th>
			<th><?php echo $this->Paginator->sort('AddressName'); ?></th>
			<th><?php echo $this->Paginator->sort('StreetAddress'); ?></th>
			<th><?php echo $this->Paginator->sort('ZipID'); ?></th>
			<th><?php echo $this->Paginator->sort('CityID'); ?></th>
			<th><?php echo $this->Paginator->sort('StateID'); ?></th>
			<th><?php echo $this->Paginator->sort('CountryID'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php foreach ($addresses as $address): ?>
	<tr>
		<td><?php echo h($address['Address']['AddressID']); ?>&nbsp;</td>
		<td><?php echo h($address['Address']['AddressName']); ?>&nbsp;</td>
		<td><?php echo h($address['Address']['StreetAddress']); ?>&nbsp;</td>
		<td><?php echo h($address['Address']['ZipID']); ?>&nbsp;</td>
		<td><?php echo h($address['Address']['CityID']); ?>&nbsp;</td>
		<td><?php echo h($address['Address']['StateID']); ?>&nbsp;</td>
		<td><?php echo h($address['Address']['CountryID']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $address['Address']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $address['Address']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $address['Address']['id']), null, __('Are you sure you want to delete # %s?', $address['Address']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Address'), array('action' => 'add')); ?></li>
	</ul>
</div>
